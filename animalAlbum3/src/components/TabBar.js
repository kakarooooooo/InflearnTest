export default function TabBar({ $app, initialState, onClick }) {
  ...
  this.render = () => {
    ...
    let $currnetTab = document.getElementById(this.state);
    $currnetTab ? ($currnetTab.className = "clicked") : ""; // $currentTab && $currentTab.className = "clicked"

    const $tapbar = this.$target.querySelectorAll("div");
    $tapbar.forEach((elm) => {
